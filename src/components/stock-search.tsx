...
export function StockSearch({ selectedTickers, onSelect }: StockSearchProps) {
  ...
  return (
    <div ref={containerRef} className="relative w-full max-w-xl mx-auto">
      <div
        className={cn(
          "flex items-center gap-3 px-4 py-3 bg-white border rounded-xl shadow-sm transition-all",
          "focus-within:shadow-md focus-within:border-zinc-400",
          atLimit && "opacity-60"
        )}
      >
        {isLoading ? (
            ...
            <ul className="max-h-72 overflow-y-auto">
              {filteredResults.map((result) => (
                <li key={result.symbol}>
                  ...
                  >
                    <span className="font-semibold text-sm bg-zinc-100 text-zinc-700 px-2 py-0.5 rounded-md shrink-0">
                      {result.symbol}
                    </span>
